...
const OpenNotificationsButton = () => {
    ...
    useEffect(() => {
        async function getNotifs() {
            ...
            await getUserNotifications(user.uid).then(res => {
                ...
                dispatch(notificationsActions.setNotifications(Object.values(res).sort((a, b) =>
    ...
    const readAll = () => {
        ...
        notifications.filter((notif: { read: any; }) => 
            !notif.read).forEach((notif: { uid: string; }) => {
            readNotification(user.uid, notif.uid)
        })
        dispatch(notificationsActions.setNotifications(notifications.map((notif: any) => 
            ({...notif, read: true}))));
    }
